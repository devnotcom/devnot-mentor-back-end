//using DevnotMentor.Business;
using DevnotMentor.Business.Repository.Interfaces;
using System;
using Xunit;

namespace DevnotMentor.Test
{
    public class MentorServiceTests
    {
        [Fact]
        public void CreateMentor_ForNewMentor()
        {
            //var userService = new UserService();
            //var user = userService.Register(new Models.RegisterUserModel { 
            //    Name = "Unit test",
            //    UserName = "random_" + new Random().Next(1000,9999) + "@devnot.com",
            //    Password = "123"
            //});

        }

        [Fact]
        public void CreateMentor_ForExistingMentor()
        {
        }

        [Fact]
        public void CreateMentor_WithNewTag()
        {
        }

        [Fact]
        public void CreateMentor_WithExistingTag()
        {
        }
    }
}
