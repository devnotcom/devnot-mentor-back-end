...
namespace DevnotMentor.Data
{
    public class MenteeRepository : BaseRepository<Mentee>, IMenteeRepository
    {
        ...
        public Task<List<Mentee>> SearchAsync(SearchRequest request)
        {
            ...
            var queryableMentee = DbContext.Mentees.AsQueryable();
        ...
        {
            return await DbContext.Mentees.Where(i => i.UserId == userId).FirstOrDefaultAsync();
        }

        public async Task<Mentee> GetByUserNameAsync(string userName)
        {
            return await DbContext.Mentees.Include(x => x.User).Where(i => i.User.UserName == userName).FirstOrDefaultAsync();
        }

        public async Task<int> GetIdByUserIdAsync(int userId)
        {
            return await DbContext.Mentees.Where(i => i.UserId == userId).Select(i => i.Id).FirstOrDefaultAsync();
        }

        public async Task<bool> IsExistsByUserIdAsync(int userId)
        {
            return await DbContext.Mentees.AnyAsync(i => i.UserId == userId);
        }

        public async Task<IEnumerable<Mentor>> GetPairedMentorsByMenteeIdAsync(int menteeId)
        {
            return await DbContext.Mentorships.Where(x => x.MenteeId == menteeId)
                .Include(x => x.Mentor)
        ...
        {
            return await DbContext.Mentees.Include(x=>x.User).Where(x => x.Id == menteeId).FirstOrDefaultAsync();
        }
    }
}
